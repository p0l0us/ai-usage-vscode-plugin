...
- `aiUsage.chatChips.enabled`: toggle the agent's icon chip beneath the chat input (default on). Clicking the chip
  opens a dialog with the same text as the status bar tooltip.
